Param(
	...
	[string]$RepoUrl="localhost:41016",
    [Parameter(Mandatory=$False)]
	[string]$AuthUrl="localhost:41017",
    [Parameter(Mandatory=$False)]
...
$params = "run", "--rm", "--network", "sensenet-auth", "mcr.microsoft.com/mssql-tools:latest", "/opt/mssql-tools/bin/sqlcmd", "-S", $DataContainer, "-d", $DataSource, "-U", $user, "-P", $password, "-Q", $query
...
$params = "run", "--rm", "--network", "sensenet-auth", "mcr.microsoft.com/mssql-tools:latest", "/opt/mssql-tools/bin/sqlcmd", "-S", $DataContainer, "-d", $DataSource, "-U", $user, "-P", $password, "-Q", $clientappScript
...
$params = "run", "--rm", "--network", "sensenet-auth", "mcr.microsoft.com/mssql-tools:latest", "/opt/mssql-tools/bin/sqlcmd", "-S", $DataContainer, "-d", $DataSource, "-U", $user, "-P", $password, "-Q", $clientsecretScript
Invoke-Cli -execFile "docker" -params $params -ErrorAction Stop
